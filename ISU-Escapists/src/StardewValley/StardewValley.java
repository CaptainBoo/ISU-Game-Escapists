...
public class StardewValley extends JPanel implements Runnable, KeyListener, MouseListener {
	// To do. Get that parsnip collecting working very important. I just wrote
	// almost 400 lines of code in one day.
	// Added the different frames for tools, seeds, plant frames, growing system,
	// watering system, inventory system.
	...
	// Booleans
	boolean walking, walkingChange, cords, playerFront = true, showInventory = true, rightClickKey, useItem, waitTime,
			twoDirectionLeft, twoDirectionRight, waterWait1, waterWait2, drawAddParsnip, drawAddGold, shopping,
			onParsnip, showInstructions;
	...
	// Description: Changes the cursor according to what cursor should be shown at
	// what time
	...
	// Description: Keeps the player from going out of bounds when in the house.
	// Also when outside checks if my cursor
	// is on the door to check if going inside. This code is technically the same as
	// isCollision
		...
		if (screen.equals("house")) {
			...
			if (player.y < 360 && (player.x < 800 && player.x > 775)) {
				...
				player.x = 535;
					...
					&& ((player.x >= 415 && player.x <= 500) || (player.x) >= 650 && player.x < 760)) {
				player.y = 360;
			}
	...
	// Description: Take the pixel location of the cursor and convert it to its
	// location in our map array
	// Parameters: None
	...
	public void update() {
		...
		if (left) {
			...
			// This !up and !down just prevents my character from doing weird movements when
			// diagonal
			if (!up && !down) {
				if (frameCount % 8 == 0) {
					characterFrame++;
	...
	// Description: Draw the image that was recieved by getToolImage() display it
	// for 30 frames
	// Paremeters: None
	// Return: Void
			...
			for (int col = 0; col < map[row].length; col++) {
				if (map[row][col] == 1) {
					if (row == treeRemoveRow && col == treeRemoveCol) {
						offScreenBuffer.drawImage(tree[treeImageFrame], col * screenWidth / 42 - 25,
								row * screenWidth / 42 - 140, this);

					} else {

						offScreenBuffer.drawImage(tree[0], col * screenWidth / 42 - 25, row * screenWidth / 42 - 140,
								this);
					}
				}
				if (map[row][col] == 3) {
				...
				if (4 <= map[row][col] && map[row][col] <= 7) {
					offScreenBuffer.drawImage(wood[map[row][col] - 4], col * screenWidth / 42 + 5,
							row * screenWidth / 42 + 14, this);
				}

				if (map[row][col] == 8) {
					offScreenBuffer.drawImage(dryPlantHole, col * screenWidth / 42 - 6, row * screenWidth / 42 + 12,
							this);
				}
				if (map[row][col] == 9) {
					offScreenBuffer.drawImage(wetPlantHole, col * screenWidth / 42 + 2, row * screenWidth / 42 + 9,
							this);
				}
	...
	// Description: Each object has a different interaction depending on what the
	// item is so this
	...
	// Description: Loop through my plant times 2d array with the time each parsnip
	// has been growing
	// And increment it
	// Parameters: None
	// Return: Void
	...
	// Description: Calculate what stage parsnip is at and the coordinates to draw
	// it at
	...
	// Description: Calculate what stage bean stalks are at and the coordinates to
	// draw it at
	// Paramenters: int row, int col
	// Return: int array with the stage at index 0, col as index 1, row as index 2
	...
	// Description: For each inventory item return the image of that item and if it
	// is a parsnip, waterCar, or parsnipSeed
	// return the coordinates for it's other stuff as well.
	// Parameters: int inventoryIndex which is the current index our inventory is
	// on, xCoord and yCoord, where the images will be drawn
	...
	// Description: add either a parsnip or parsnip seed to our inventory. Increment
	// the amount if exists or add it entirely if it doesn't
	...
	// Description: Draw the gold numbers cuz each digit is drawn individually at
	...
	public void images() {
		...
		try {
			...
			outside = outsideImagePreScale.getScaledInstance((int) Math.round(outsideImagePreScale.getWidth() * 1.5),
					(int) Math.round(outsideImagePreScale.getHeight() * 1.5), Image.SCALE_SMOOTH);
		} catch (Exception e) {
		}
		try {
...
class Player {// extends MapObject {
	...
	// Description: is the same as keepInBounds() keeps my player from running into
	// objects on the map and walking
	// off the map or into water and other stuff
	// Parameters: map, dir, up,down,left,right,twoDirectionLeft,twoDirectionRight
	// bascailly just the map and anything related to the characters
	// direction to easily calculate what is in front of where the player is going.
	// Return: None
	public boolean isCollision(int[][] map, String dir, boolean up, boolean down, boolean left, boolean right,
			boolean twoDirectionLeft, boolean twoDirectionRight) {
		int arrRow = 0, arrCol = 0;
	...
	// Description: Checks if the player is in front of the objects or not. This is
	// because when the player
	// is behind the objects he should be drawn that way as well to make the objects
	// look 3d.
	// Parameters: map, dir (the map we're checking through, and dir to see what
	// tile to check
	...
	public boolean isPlayerInFrontMapObject(int[][] map, String dir) {
		...
		// This is because if the player crashes into the border boundary the code
		// crashes cuz index -1
		try {
